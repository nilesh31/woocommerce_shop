...
// display title on product page define the woocommerce_before_shop_loop_item callback   
...
// remove action for remove title on bottom//////////
remove_action('woocommerce_shop_loop_item_title', 'woocommerce_template_loop_product_title', 10);

//display pagination top and bottom hook/////////
add_action('woocommerce_after_shop_loop', 'woocommerce_pagination', 10);

add_action('woocommerce_before_shop_loop', 'woocommerce_pagination', 10);

//display image gallery zoom function hook/////////
function image_gallery_zoom() {
    add_theme_support('wc-product-gallery-zoom');
    add_theme_support('wc-product-gallery-lightbox');
    add_theme_support('wc-product-gallery-slider');
}

add_action('after_setup_theme', 'image_gallery_zoom');
//function and hook end/////
//display tabs after add to cart hook/////////
remove_action('woocommerce_after_single_product_summary', 'woocommerce_output_product_data_tabs', 10);
add_action('woocommerce_after_add_to_cart_button', 'woocommerce_output_product_data_tabs', 10);

//remove reviews tab function and hook/////////
...
//remove reviews tab hook/////////
add_filter('woocommerce_product_tabs', 'remove_reviews_tab', 98);

//display reviews on a shop page..// 
function show_reviews() {

    comments_template();
}

//display reviews on a shop page hook..//
add_action('woocommerce_after_single_product_summary', 'show_reviews', 10);

function function_check_login_redirect($user_login, $user) {

    global $current_user;
    get_currentuserinfo();
    $codename = $user->user_login;
    $date = date('d-m-Y');
    $user_id = $user->ID;
    $last_user_login_date = get_user_meta($user_id, 'last_login', TRUE);
    if ($date === $last_user_login_date) {
...
// Add meta information code here
...
function user_registration($user_id) {
    ...
    $user_name_id = get_userdata($user_id);
    $codename = $user_name_id->user_login;
    $date = date('d-m-Y');
...
function delete_coupan_user_logout() {
    global $current_user;
    get_currentuserinfo();
    $user_coupon_code = $current_user->user_login;
    $user_coupon_data = new WC_Coupon($user_coupon_code);
    if (!empty($user_coupon_data->id)) {
...
//this is hook for click apply coupon code
add_action('woocommerce_applied_coupon', 'get_coupons', 20);

//=========================cart discount code here========================================//
...
function wc_user_cart_discount() {
    ...
    $coupon_code = $current_user->user_login; //coupon generat with login user
    $user_coupons = WC()->cart->get_coupons(); //get all coupon in variable
    //if have coupon than go inside loop
    if (!empty($user_coupons)) {
        foreach ($user_coupons as $coupon) {
            $user_discount += get_post_meta($coupon->id, 'coupon_amount', TRUE);
        }
    }
